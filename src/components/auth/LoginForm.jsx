...
import { Mail, Lock, AlertCircle, Eye, EyeOff, Car, Zap, Shield } from "lucide-react";
...
export const LoginForm = () => {
  ...
  const getErrorMessage = (err) => {
    ...
    if (err.status === 401) {
      // Check for suspended account (Vietnamese or English)
      if (backendMsgLower.includes('khóa') || 
          backendMsgLower.includes('suspended') ||
          backendMsgLower.includes('tạm khóa')) {
      ...
      // Check for deleted account (Vietnamese or English)
      if (backendMsgLower.includes('xóa') || 
          backendMsgLower.includes('deleted') ||
          backendMsgLower.includes('đã bị xóa')) {
        return "Tài khoản của bạn đã bị xóa. Vui lòng liên hệ quản trị viên qua gmail Evmarker@gmail.com để biết thêm chi tiết.";
      }
      
      // Check for invalid credentials (English messages from backend)
      if (backendMsgLower.includes('invalid email or password') ||
          backendMsgLower.includes('invalid') ||
          backendMsgLower.includes('unauthorized')) {
        return "Email hoặc mật khẩu không đúng. Vui lòng thử lại.";
      }
      
      // Return Vietnamese version of backend message if available
      return backendMsg || "Email hoặc mật khẩu không đúng. Vui lòng thử lại.";
    }
